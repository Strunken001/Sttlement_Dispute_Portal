            ...
            if (supportAgent.supportAgentId == Guid.Empty)
            {
                try
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        $"Dear User, Your account has been created with username:'{supportAgent.Email}'  and temporary  password:'{randomPassword.ToString()}' .Please confirm your account by clicking this link: <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>link</a>");
