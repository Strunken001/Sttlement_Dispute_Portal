...
namespace src.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private readonly IADLogin _adlogin;
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (model.Email.Contains("coralpay.com"))
                {
                    //var content = new ADLogin();

                    res = _adlogin.verifyADUser(model.Email, model.Password, "1");
                    ...
                    //else
                    //{
                    //    ModelState.AddModelError(string.Empty, "Invalid login attempt. User is not on AD");
                    //    ViewData["error"] = "Invalid login attempt. User is not on AD";
                    //    return View(model);
                    //}
                }
                //need to activate email first
                bool isEmailActivated = await _dotnetdesk.IsAccountActivatedAsync(model.Email, _userManager);
                if (res == false && !isEmailActivated)
                {
                ...
                else if (res == true && result.IsNotAllowed != true)
                {
                    string[] FullName = model.Email.Split('.') ;
                    string lastName = FullName[1].ToString();
                    lastName = lastName.Replace("@coralpay", "");
                    RegisterViewModel reg = new RegisterViewModel();
                    reg.Email = model.Email;
                    reg.FullName = FullName[0].ToString() + " " + FullName[1].ToString();
                    reg.Password = model.Password;
                    reg.ConfirmPassword = model.Password;
                    var response = await Register(reg, "");
